...
type BinanceTable struct {
	ID   primitive.ObjectID `json:"id"           form:"_id"         bson:"_id"          desc:"ID"`
	Coin string             `json:"coin"         form:"coin"        bson:"coin"         desc:"coin"`
}

const BinanceDBTableName = "binance"

func (*BinanceTable) TableName() string {
	return BinanceDBTableName
}

func (m *BinanceTable) Save() error {
	lock.Lock()
	defer lock.Unlock()
	var err error
	if m.ID.IsZero() {
...
func (m *BinanceTable) Delete() error {
